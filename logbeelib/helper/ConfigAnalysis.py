    ...
    def read(self,configfilename):
        if os.path.exists(configfilename):
            if os.path.isfile(configfilename):
                _ = [self.__genarate_config(item[0],item[-1]) for item in  self.__analysislines(self.__analysisfile(configfilename))]
            ...
            for n in range(len(clines)):
                for nkey in self.__update_log.keys():
                    if re.match(r"^%s( |\t)*\=( ||\t)*(\S|.)*$"%nkey, clines[n]):
                        clines[n]=re.sub(r"^%s( |\t)*\=( ||\t)*(\S|.)*$"%nkey, "%s = %s"%(nkey,self.__update_log[nkey]), clines[n])
            if not re.match(r'.*(\r\n|\r|\n)$', clines[-1]):
                clines[-1]=clines[-1]+"\n"
